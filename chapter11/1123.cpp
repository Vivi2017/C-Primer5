...
using namespace std;
void print_child(multimap<string, std::vector<std::string> > familyMap)
{
...
void add_family(multimap<string, std::vector<std::string> > &familyMap, string fn){
...
void add_child(multimap<string, std::vector<std::string> > &familyMap, string fn, string cn){
	
	    auto it = familyMap.find(fn);
		if (it != familyMap.end()) {
				cout << " Family "<< fn <<" add new child ! "<< cn <<endl;
			    it->second.push_back(cn);  
		}else{
...
int main(){
		multimap<string, std::vector<std::string> > familyMap;
